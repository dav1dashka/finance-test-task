import tickersSlice, { setTickersData } from './tickersSlice';

describe('tickersSlice reducers', () => {
    test('set tickers data correctly', () => {
        const initialState = {
            previousData: [],
            currentData: []
        };
        ...
        const action = setTickersData(newData);

        const newState = tickersSlice(initialState, action);

        expect(newState.currentData).toEqual(newData);
        expect(newState.previousData).toEqual(initialState.currentData);
    });
});
